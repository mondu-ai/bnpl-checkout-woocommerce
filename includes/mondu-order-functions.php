...
function order_data_from_wc_order( WC_Order $order ) {
  ...
  $order_data = array(
    ...
    'buyer'                 => array(
      'first_name'            => isset( $billing_first_name ) && not_null_or_empty( $billing_first_name ) ? $billing_first_name : null,
      'last_name'             => isset( $billing_last_name ) && not_null_or_empty( $billing_last_name ) ? $billing_last_name : null,
      'company_name'          => get_company_name_from_wc_order( $order ),
      'email'                 => isset( $billing_email ) && not_null_or_empty( $billing_email ) ? $billing_email : null,
      'phone'                 => isset( $billing_phone ) && not_null_or_empty( $billing_phone ) ? $billing_phone : null,
      'external_reference_id' => isset( $customer_id ) && not_null_or_empty( $customer_id ) ? (string) $customer_id : null,
      'is_registered'         => is_user_logged_in(),
    ),
    'billing_address'       => array(
      'address_line1' => isset( $billing_address_line1 ) && not_null_or_empty( $billing_address_line1 ) ? $billing_address_line1 : null,
      'address_line2' => isset( $billing_address_line2 ) && not_null_or_empty( $billing_address_line2 ) ? $billing_address_line2 : null,
      ...
      'zip_code'      => isset( $billing_zip_code ) && not_null_or_empty( $billing_zip_code ) ? $billing_zip_code : null,
      'country_code'  => isset( $billing_country_code ) && not_null_or_empty( $billing_country_code ) ? $billing_country_code : null,
    ),
...
function get_shipping_address_from_order( WC_Order $order ) {
  ...
  if ( $order->needs_shipping_address() ) {
    $shipping_data = array(
      'address_line1' => isset( $shipping_address_line1 ) && not_null_or_empty( $shipping_address_line1 ) ? $shipping_address_line1 : null,
      'address_line2' => isset( $shipping_address_line2 ) && not_null_or_empty( $shipping_address_line2 ) ? $shipping_address_line2 : null,
      ...
      'zip_code'      => isset( $shipping_zip_code ) && not_null_or_empty( $shipping_zip_code ) ? $shipping_zip_code : null,
      'country_code'  => isset( $shipping_country_code ) && not_null_or_empty( $shipping_country_code ) ? $shipping_country_code : null,
    );
  } else {
    $shipping_data = array(
      'address_line1' => isset( $billing_address_line1 ) && not_null_or_empty( $billing_address_line1 ) ? $billing_address_line1 : null,
