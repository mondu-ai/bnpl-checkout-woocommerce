...
class MonduRequestWrapper {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_before_order_object_save', array( $this, 'update_order_if_changed_some_fields' ), 10, 1 );
	...
	public function create_order( WC_Order $order, $success_url ) {
		...
		update_post_meta( $order->get_id(), ORDER_ID_KEY, $mondu_order['uuid'] );
		return $mondu_order;
	}
	...
	public function get_order( $order_id ) {
		...
		$mondu_order_id = get_post_meta( $order_id, ORDER_ID_KEY, true );
	...
	public function ship_order( $order_id ) {
		...
		update_post_meta( $order_id, INVOICE_ID_KEY, $invoice['uuid'] );
		return $invoice;
	}
	...
	public function get_invoice( $order_id ) {
		...
		$mondu_order_id   = get_post_meta( $order_id, ORDER_ID_KEY, true );
		$mondu_invoice_id = get_post_meta( $order_id, INVOICE_ID_KEY, true );
	...
	public function order_refunded( $order_id, $refund_id ) {
		...
		$mondu_invoice_id = get_post_meta( $order->get_id(), INVOICE_ID_KEY, true );
		if ( ! $mondu_invoice_id ) {
			Mondu_WC()->log(
				array(
