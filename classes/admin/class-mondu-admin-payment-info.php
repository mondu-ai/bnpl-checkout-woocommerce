...
if ( ! class_exists( 'MonduAdminPaymentInfo' ) ) {
    class MonduAdminPaymentInfo {
        /**
         * Order
         *
         * @var WC_Order
        ...
        public function get_mondu_section_html() {
            if ( ! in_array( $this->order->get_payment_method(), PAYMENT_METHODS, true ) ) {
                return null;
            }
            ...
            if ( $this->order_data ) {
                ...
                <section class="woocommerce-order-details mondu-payment">
                    ...
                    <?php
                    if ( in_array( $this->order_data['state'], array( 'confirmed', 'partially_shipped' ), true ) ) {
                        ?>
                        <?php
                        $mondu_data = array(
        ...
        private function get_invoices() {
            try {
                return Mondu_WC()->mondu_request_wrapper->get_invoices( $this->order->get_id() );
        ...
        private function get_order() {
            try {
                return Mondu_WC()->mondu_request_wrapper->get_order( $this->order->get_id() );
            } catch ( ResponseException $e ) {
                return array();
            }
        }
